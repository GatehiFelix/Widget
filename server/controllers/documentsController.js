...
export const uploadDocumentController = asyncHandler(async (req, res) => {
  ...
  tenant_id,                                       
  product_id: Number(tenant_id.replace('tenant_', '')),
  ...
  processDocumentJob({
    ...
    logger.error(`Unhandled worker error for job ${doc.job_id}: ${err.message}`);
  });
});
...
export const getDocumentStatsController = asyncHandler(async (req, res) => {
  ...
  const [qdrantStats, allDocs, topDocs, recentActivity] = await Promise.all([
    ...
    ClientDocument.findAll({
      where: { tenant_id },
      attributes: ['id', 'name', 'size', 'type', 'status', 'chunk_count', 'query_count', 'uploaded_at', 'indexed_at'],
    }),
    ...
    ClientDocument.findAll({
      ...
      limit: 10,
      attributes: ['id', 'name', 'type', 'status', 'uploaded_at', 'indexed_at', 'error_message'],
    }),
  ]);
  ...
  res.status(200).json({
    ...
    data: {
      ...
      topDocuments: topDocs.map(d => ({
        id: d.id, name: d.name, type: d.type, queryCount: d.query_count,
      })),
      typeBreakdown,
      recentActivity: recentActivity.map(doc => ({
        ...
        event: doc.status === 'processing' ? `Processing ${doc.name}`
             : doc.status === 'indexed'    ? `Indexed ${doc.name}`
             :                               `Failed ${doc.name}`,
        color: doc.status === 'indexed' ? 'green'
             : doc.status === 'failed'  ? 'red' : 'blue',
...
export const getJobStatusController = asyncHandler(async (req, res) => {
  ...
  const doc = await ClientDocument.findOne({
    where: { job_id },
    attributes: ['id', 'job_id', 'name', 'status', 'chunk_count', 'error_message', 'indexed_at', 'uploaded_at'],
  });
  ...
  const stageMap = {
    ...
    failed:     { stage: 'error',      percent: 0,   label: doc.error_message || 'Failed' },
  };
