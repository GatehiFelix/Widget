...
import { Client, RefreshToken } from "../models/index.js";
...
export const startSession = asyncHandler(async (req, res) => {
  ...
  agentClient.joinConversation(session.roomId)

  console.log("session object:", session);
  console.log("session.roomId:", session.roomId);

  const accessToken = generateAccessToken(session.roomId, clientId);
  const refreshToken = generateRefreshToken(session.roomId, clientId);

  try {
    await RefreshToken.upsert({
      token: refreshToken,
      session_id: String(session.roomId),
      tenant_id: clientId,
      expires_at: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000), // 7 days
    });
  } catch (err) {
    console.error(
      "RefreshToken create failed:",
